...
def save_brats_nifti(seg_map: Tensor, names: list, mode: str, data_root: str, save_epoch_path: str, logger=None):
    """
    Save NIfTI predicted segmentation maps and print (or log) the save path.
    """
    ...
    for b in range(B):
        ...
        original_img_path = join(data_root,  'BraTS2021_Training_Data',names[b], names[b] + "_t1.nii.gz")
        affine = nib_affine(original_img_path)

        save_path = join(save_epoch_seg_path, names[b] + "_pred.nii.gz")

        nib.save(nib.Nifti1Image(seg_img, affine), save_path)
