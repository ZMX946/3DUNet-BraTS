...
class RobustZScoreNormalization(MapTransform):
    def __call__(self, data):
        ...
        for key in self.key_iterator(d):
            ...
            lower_t = torch.as_tensor(lower, device=d[key].device, dtype=d[key].dtype)
            upper_t = torch.as_tensor(upper, device=d[key].device, dtype=d[key].dtype)

            d[key][mask & (d[key] < lower_t)] = lower_t
            d[key][mask & (d[key] > upper_t)] = upper_t
